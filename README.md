...
- 🐧 **3 systèmes** (images plateforme OCI) : Ubuntu 24.04, Oracle Linux 9 et **Windows Server 2022**.
...
- ⚙️ **Provisioning OCI Compute automatique** et idempotent via un **réconciliateur** cron.
- 🔑 **Accès sécurisé par VM** : clé SSH **RSA chiffrée AES-GCM** (Linux, générée dans le Worker) ou
  **mot de passe RDP** chiffré (Windows) — propriétaire uniquement. Guides intégrés (MobaXterm, Termius, Bureau à distance).
- 💾 **Snapshots de boot volume** : créer / supprimer, **snapshot auto avant suppression**, **restaurer**
  une VM depuis un snapshot à la création.
- ⏱️ **Cycle de vie** : dates obligatoires, **suppression auto à l'échéance**, **arrêt sur
  inactivité** (CPU OCI Monitoring), extinction nocturne, **planification** start/stop par VM.
- 🛡️ **Durcissement sécurité** : DNS filtré (Cloudflare for Families), blocage P2P/torrent, hostname
  verrouillé, **+ egress de la security list verrouillé** (filtrage réseau non contournable).
...
npx wrangler d1 migrations apply git_vm_oracle --local
...
Navigateur ──HTTPS──> Cloudflare Worker (Hono)
   ...
   │                       ├──> OCI Compute (signature RSA, Web Crypto) = provisioning réel
   │                       └──> EmailJS = notifications
   └─ La cron réconcilie en continu l'état réel OCI avec la DB (provisioning→active, drift, expiry).
```
...
```
src/            Worker Cloudflare (API, OIDC, OCI, email, D1, cron)
...
| [docs/CONFIGURATION.md](docs/CONFIGURATION.md) | Variables, secrets, IAM OCI, Entra, EmailJS, rotation |
