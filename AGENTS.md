...
- **Prod** : <https://git-vm-oracle.satom-openstack.workers.dev>
- **Repo** : <https://github.com/Thomas-TP/GIT-VM-Oracle>

## 2. Règles d'or (NE PAS enfreindre)

1. **Ne casse pas l'existant qui marche.** Le socle OCI + Workers est déployé et fonctionnel. On
...
4. **Aucun secret dans le repo.** Secrets via `wrangler secret put` uniquement. Les scripts OCI lisent
   les creds depuis l'environnement (`$env:OCI_PRIVATE_KEY_FILE`, `OCI_TENANCY`, `OCI_USER`…), jamais en dur.
...
src/                      WORKER (API + cron)
  ...
  oci.ts                  Client OCI + Web Crypto (RSA-SHA256) : launch/describe/start/stop/reboot/
...
**La DB = état désiré.** Une **cron Cloudflare `*/2 * * * *`** (compte **satom.ch**, qui a des slots de
cron libres — contrairement à git.swiss, plafonné à 5) exécute `runReconcile()` pour réconcilier le réel
OCI avec la DB, en séquence :
*(Déclenchement manuel possible via `POST /api/internal/reconcile`, gardé par `RECONCILE_TOKEN`.)*
...
5. `enforceIdleStop` — **arrêt sur inactivité** : CPU OCI Monitoring max < 10% sur `IDLE_STOP_HOURS`
   (déf. 3 h, historique suffisant requis) → stop + notif. L'utilisateur peut relancer.
6. `syncSnapshots` — suit les boot volume backups en cours (`pending → completed/error`).
...
- `snapshots(id, request_id, user_email, aws_snapshot_id, description, root_device, architecture,
  size_gb, status, os, created_at, completed_at, …)` — backup de boot volume, restauration, snapshot-avant-suppression.
...
Une demande = **PERF × STORAGE × OS** (+ cours optionnel). Compte OCI sur **crédits gratuits** avec un
**budget cible 100 CHF** + alertes 20/50/100 (`scripts/oci-budget.mjs`). `PERF` = **shapes E-Flex x86**
(AMD, OCPU/RAM configurables, avec fallback de capacité E5→E4→E3→Standard3) ; les images sont des **OCID
plateforme `eu-zurich-1` vérifiés** (`scripts/oci-images.mjs`). OS (une entrée par famille) : Ubuntu 24.04,
Oracle Linux 9, **Windows Server 2022** — Windows en RDP (utilisateur `opc`), Linux en SSH (clé **RSA**
générée par le Worker). Voir [ADR 0007](docs/adr/0007-catalogue-os-et-windows-rdp.md).

**Bundles d'outils par cours** (`COURSES`) : préinstalle des logiciels via cloud-init (Linux) /
cloudbase-init `#ps1_sysnative` (Windows) au premier boot ; callback `course-done` → l'UI affiche « outils prêts ».
...
- **Snapshots de boot volume** : créer / lister / **supprimer** ; **snapshot auto avant suppression** ; **restaurer**
  une VM depuis un snapshot à la création (boot volume recréé depuis le backup). Onglet « Snapshots & export » de la VM.
  [ADR 0009](docs/adr/0009-snapshots-et-restauration-locale.md). (L'export VMware/VirtualBox a été retiré.)
- **Nom de VM** : obligatoire ; `displayName` OCI = `<nom>.<préfixe-email>` (ex. `python.thomas.prudhomme`).
- **Arrêt sur inactivité** : OCI Monitoring CPU (`IDLE_STOP`).
- **Durcissement sécurité** (`HARDENING`) : in-VM (DNS Cloudflare for Families, blocage P2P/torrent,
  hostname verrouillé — Linux + Windows) **+** verrouillage **egress de la security list** (couche réseau
  non contournable, `scripts/oci-harden.mjs`). Voir §11.
...
npx wrangler d1 migrations apply git_vm_oracle --local   # migrations locales
...
npx wrangler tail git-vm-oracle --format pretty    # logs prod
```
...
- **Réseau (la vraie barrière)** : `scripts/oci-harden.mjs` verrouille l'**egress de la security
  list partagée** en liste blanche (80/443, **DNS 53 → Cloudflare uniquement**, NTP, SSH, **link-local
  OCI 169.254/16** pour metadata/iSCSI) + default-deny. Bloque torrents/P2P/sites X au niveau réseau,
  **force le DNS filtré**, et un root ne peut **pas** le contourner. À lancer une fois avec des creds.
- ⚠️ Effet de bord : le 53 est bloqué sauf vers Cloudflare → une VM **antérieure** au durcissement
  (DNS = résolveur OCI) perd la résolution ; régler son `resolv.conf` sur 1.1.1.3 ou la recréer.
...
`npx wrangler d1 migrations apply git_vm_oracle --remote && npx wrangler deploy`. → **Les migrations
...
- **Windows RDP** : port **3389** ouvert sur la security list (par `scripts/oci-setup.mjs`). Le
  durcissement egress (`scripts/oci-harden.mjs`) restreint la **sortie** — ne touche pas l'entrée 22/3389.
...
- **Images** : OCID régionaux figés dans `presets.ts`, ils périment → `scripts/oci-images.mjs`.
- **Idle stop inerte** sans le droit OCI **read metrics** (Monitoring) sur l'utilisateur API.
- **Capacité OCI** : « Out of host capacity » fréquent sur compte trial → `launchInstance` essaie
  plusieurs shapes E-Flex x86 (fallback) avant d'échouer.
